...
class CompilerWithDynamicFooter:
    """
    ...
    def __init__(self, interface_class: typing.Type[Interface], NC_filename, movement_speed, cutting_speed, pass_depth,
        ...
        self.interface.set_NCfilename(NC_filename)
        ...
        self.header = custom_header + [self.interface.dynamic_header()]
        self.footer = custom_footer
        self.body = []

    def set_NCfilename(self, NCfilename):
        self.interface.set_NCfilename(NCfilename)
        ...
        for i in range(passes):
            ...
            if i < passes - 1:  # If it isn't the last pass, turn off the laser and move down
                ...
                if self.pass_depth > 0:
                    ...
                    gcode.append(self.interface.linear_move(
                        z=-self.pass_depth))
                    gcode.append(self.interface.set_absolute_coordinates())
        gcode.append(self.interface.dynamic_footer())
    ...
    def append_line_chain(self, line_chain: LineSegmentChain):
        ...
        if self.interface.position is None or abs(self.interface.position - start) > TOLERANCES["operation"]:
            ...
            code = [self.interface.laser_off(), self.interface.set_movement_speed(self.movement_speed),
                    self.interface.linear_move(
                        start.x, start.y), self.interface.set_movement_speed(self.cutting_speed),
